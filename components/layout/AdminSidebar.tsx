...
import { motion, AnimatePresence } from 'framer-motion' // Para el efecto suave
import {
  BarChart3,
  Briefcase,
  Calendar,
...
// --- Tipos y Configuración (Igual a tu original) ---
...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    icon: Calendar,
    children: [{ label: 'Nueva', href: '/admin/operaciones/agenda/nueva', icon: PlusCircle, description: 'Crear nueva agenda' }],
  },
  {
    label: 'Operaciones',
    icon: Settings2,
  ...
  { label: 'Clientes', href: '/admin/personas/clientes', icon: Users, children: [{ label: 'Nuevo', href: '/admin/personas/clientes/nuevo', icon: UserPlus, description: 'Registrar nuevo cliente' }] },
  { label: 'Personal', href: '/admin/personas/personal', icon: Shield, children:   [{ label: 'Nuevo', href: '/admin/personas/personal/nuevo', icon: UserPlus, description: 'Registrar nuevo personal' }] },
  { label: 'Comunicación', href: '/admin/personas/comunicacion', icon: Megaphone },
  {
    label: 'Finanzas',
    href: '/admin/finanzas',
    icon: Wallet,
...
// --- Componente de Item con HOVER ---
function DesktopNavItem({ item, pathname }: { item: NavItem; pathname: string }) {
  const [isHovered, setIsHovered] = useState(false)
  const active = (item.href && pathMatches(pathname, item.href)) || item.children?.some(c => pathMatches(pathname, c.href))
  const Icon = item.icon

  // Tu clase exacta de botón
  const buttonClass = `
  ...
  return (
    <div 
      className="relative py-4" // Zona para que no se cierre el menu al bajar el mouse
    ...
    >
      <Link href={item.href || '#'} className={buttonClass}>
        ...
        {item.children && <ChevronDown className={`h-4 w-4 transition-transform ${isHovered ? 'rotate-180' : ''}`} />}
      </Link>
          ...
          >
            <div className="w-[340px] rounded-3xl border border-white/10 bg-[#101019]/96 p-3 shadow-[0_24px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl">
              <div className="mb-2 px-2">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-white/30">{item.label}</p>
              </div>
                ...
                {item.children.map((child) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      {child.description && <p className="mt-0.5 text-xs text-white/45">{child.description}</p>}
                    </div>
                  </Link>
                ))}
              </div>
    ...
    <div className="sticky top-0 z-40 w-full border-b border-white/10 bg-[#0b0b12]/92 backdrop-blur-2xl">
      <div className="mx-auto flex h-[78px] max-w-[1720px] items-center justify-between gap-6 px-6">
        ...
        <Link href="/admin/dashboard" className="flex shrink-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.28em] text-white/28 leading-none mb-1">RPM</p>
