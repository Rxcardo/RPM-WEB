  ...
  async function loadReporte() {
    try {
      ...
      if(tipo==='liquidacion'){
        ...
        const cuentasOrigenMap=new Map<string,any>()
        const origenIds=[...new Set([...baseRows.map((r)=>r.cliente_plan_id),...baseRows.map((r)=>r.cita_id)].filter(Boolean))] as string[]
        if(origenIds.length>0){const{data:cuentasData}=await supabase.from('cuentas_por_cobrar').select('id,cliente_id,origen_id,estado,saldo_usd,saldo_bs').in('origen_id',origenIds).not('estado','in','("anulado","anulada","cancelado","cancelada")').order('created_at',{ascending:false});for(const c of(cuentasData||[]) as any[]){const k=String(c.origen_id||'');if(k&&!cuentasOrigenMap.has(k)){const est=String(c.estado||'').trim().toLowerCase();const isPagada=['cobrada','pagada','pagado','cerrada','liquidada'].includes(est)||(Number(c.saldo_usd||0)<=0.009&&Number(c.saldo_bs||0)<=0.009);cuentasOrigenMap.set(k,{...c,_pagada:isPagada})}else if(k){const prev=cuentasOrigenMap.get(k);if(!prev._pagada){const est=String(c.estado||'').trim().toLowerCase();const isPagada=['cobrada','pagada','pagado','cerrada','liquidada'].includes(est)||(Number(c.saldo_usd||0)<=0.009&&Number(c.saldo_bs||0)<=0.009);if(isPagada)cuentasOrigenMap.set(k,{...c,_pagada:true})}}}}
        const enriched=baseRows.map((r)=>{const cuentaKey=r.cliente_plan_id?String(r.cliente_plan_id):r.cita_id?String(r.cita_id):'';const cuenta=cuentaKey?cuentasOrigenMap.get(cuentaKey):null;const cuentaPagada=!!cuenta&&!!cuenta._pagada;const baseUsdCalc=Number(r.monto_base_usd??r.base??0);const profUsdCalc=Number(r.monto_profesional_usd??r.profesional??0);const pctFisioCalc=baseUsdCalc>0&&profUsdCalc>0?(profUsdCalc/baseUsdCalc)*100:0;const necesitaCuentaPagada=pctFisioCalc>35;const esRetenida=necesitaCuentaPagada&&(!cuenta||!cuentaPagada);return{...r,cliente_nombre:r.cliente_id?clienteNombres.get(r.cliente_id)||null:null,concepto:r.tipo==='plan'&&r.cliente_plan_id?planConceptos.get(r.cliente_plan_id)||null:r.tipo==='cita'&&r.cita_id?citaConceptos.get(r.cita_id)||null:null,cuenta_pagada:cuentaPagada,es_retenida:esRetenida}})
