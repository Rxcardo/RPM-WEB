...
type Plan = { id: string; nombre: string; sesiones_totales: number; vigencia_dias: number; precio: number; estado: string; descripcion: string | null }
type ClientePlan = { id: string; cliente_id: string; plan_id: string; sesiones_totales: number; sesiones_usadas: number; fecha_inicio: string | null; fecha_fin: string | null; estado: string; created_at: string; planes: Plan | null; origen: string; porcentaje_rpm: number; monto_base_comision: number | null }
type Empleado = { id: string; nombre: string; especialidad: string | null; comision_plan_porcentaje: number; comision_cita_porcentaje: number }
type Recurso = { id: string; nombre: string; tipo: string | null }
type MetodoPago = { id: string; nombre: string; tipo: string | null }
type EntrenamientoExistente = { id: string; hora_inicio: string; hora_fin: string; fecha: string; estado: string; clientes: { nombre: string } | null }
...
function CalendarioDisponibilidad({ empleadoId, fechaVista, horaInicio, horaFin, entrenamientosExistentes, onSelectHora }: {
  empleadoId: string; fechaVista: string; horaInicio: string; horaFin: string
...
export default function ClientePlanPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [clienteRes, planesRes, planActivoRes, empleadosRes, recursosRes, metodosRes] = await Promise.all([
      ...
      supabase.from('planes').select('id, nombre, sesiones_totales, vigencia_dias, precio, estado, descripcion').eq('estado', 'activo').order('nombre'),
      supabase.from('clientes_planes').select(`id, cliente_id, plan_id, sesiones_totales, sesiones_usadas, fecha_inicio, fecha_fin, estado, created_at, origen, porcentaje_rpm, monto_base_comision, planes:plan_id ( id, nombre, sesiones_totales, vigencia_dias, precio, estado, descripcion )`).eq('cliente_id', id).eq('estado', 'activo').order('created_at', { ascending: false }).limit(1).maybeSingle(),
  ...
  useEffect(() => {
    ...
    void supabase.from('entrenamientos').select('id, hora_inicio, hora_fin, fecha, estado, clientes:cliente_id ( nombre )')
      .eq('empleado_id', empleadoId).eq('fecha', fechaVistaCal).neq('estado', 'cancelado').order('hora_inicio')
  ...
  const fechasPreview = useMemo(() => {
    ...
    const total = modo === 'renovar' ? selectedPlan.sesiones_totales + sesionesRestantes : selectedPlan.sesiones_totales
    return generarFechasSesiones(fechaInicio, diasSemana, total)
  }, [fechaInicio, diasSemana, selectedPlan, modo, sesionesRestantes])
  ...
  function nn(v: string) { const n = Number(v); return Number.isFinite(n) && n >= 0 ? n : 0 }
  ...
  function resetForm() {
    ...
    setDiasSemana([]); setHoraInicio(''); setDuracionMin(60); setMetodoPagoId('')
    setUsarPrecioPlan(true); setMontoPersonalizado(''); setRegistrarPago(true)
    setEsBs(false); setTasaBCV(''); setNotasPago(''); setMotivoCancelacion('')
    setErrorMsg(''); setSuccessMsg('')
    setMontoBaseComision(''); setMontoRpm(''); setMontoEntrenador('')
    setPagoAlCancelar(null); setFechaVistaCal(getTodayLocal())
  }
  ...
  async function crearPlanInline() {
    ...
    try {
      const { data: plan, error } = await supabase.from('planes').insert({
        nombre: nuevoPlanNombre.trim(), sesiones_totales: nuevoPlanSesiones,
        vigencia_dias: nuevoPlanVigencia, precio: Number(nuevoPlanPrecio), estado: 'activo',
      }).select('id, nombre, sesiones_totales, vigencia_dias, precio, estado, descripcion').single()
      ...
      setMontoBaseComision(String(base)); setMontoRpm(String(rpmI)); setMontoEntrenador(String(r2(base - rpmI)))
      setMostrarCrearPlan(false); setNuevoPlanNombre(''); setNuevoPlanSesiones(12); setNuevoPlanVigencia(30); setNuevoPlanPrecio('')
    } catch (err: any) { alert(err?.message || 'Error al crear el plan.') }
    finally { setCreandoPlan(false) }
  }

  async function registrarComision(clientePlanId: string, empId: string, clienteId: string, base: number, fecha: string, rpm: number, profesional: number) {
    try {
      const { error } = await supabase.from('comisiones_detalle').insert({
        empleado_id: empId, cliente_id: clienteId, cliente_plan_id: clientePlanId,
        fecha, base, profesional, rpm, tipo: 'plan', estado: 'pendiente',
      })
      if (error) console.error('❌ Error comisión:', error.message)
      else console.log('✅ Comisión:', { base, rpm, profesional })
  ...
  async function handleAsignar(e: React.FormEvent) {
    ...
    if (registrarPago && esBs && (!tasaBCV || tasaBCVNum <= 0)) { setErrorMsg('Ingresa la tasa BCV.'); return }
    ...
    try {
      ...
      const { data: np, error: cpE } = await supabase.from('clientes_planes').insert({ cliente_id: id, plan_id: plan.id, sesiones_totales: plan.sesiones_totales, sesiones_usadas: 0, fecha_inicio: fechaInicio, fecha_fin: fechaFin, estado: 'activo' }).select('id').single()
      ...
      const { error: eE } = await supabase.from('entrenamientos').insert(fechas.map((f) => ({ cliente_plan_id: np.id, cliente_id: id, empleado_id: empleadoId, recurso_id: recursoId || null, fecha: f, hora_inicio: hiN, hora_fin: hfN, estado: 'programado' })))
      if (eE) throw new Error(`Entrenamientos: ${eE.message}`)
      if (registrarPago && montoBase > 0) {
        const mF = esBs ? montoCalculadoBs : montoBase
        const concepto = esBs ? `Plan: ${plan.nombre} — ${cliente?.nombre} (${formatMoney(montoBase)} × ${tasaBCV} = ${formatBs(montoCalculadoBs)})` : `Plan: ${plan.nombre} — ${cliente?.nombre}`
        const { error: pE } = await supabase.from('pagos').insert({ fecha: fechaInicio, tipo_origen: 'plan', cliente_id: id, cliente_plan_id: np.id, concepto, categoria: 'plan', monto: mF, metodo_pago_id: metodoPagoId, estado: 'pagado', notas: notasPago || null })
  ...
  async function handleRenovar(e: React.FormEvent) {
    ...
    const plan = selectedPlan!
    const sesN = plan.sesiones_totales + sesionesRestantes
    const baseC = r2(nn(montoBaseComision || String(plan.precio || 0)))
    const rpmV = r2(nn(montoRpm))
    const entV = r2(nn(montoEntrenador))
    if (baseC > 0 && r2(rpmV + entV) !== baseC) {
    ...
    try {
      ...
      const fechaFin = addDaysToDate(fechaInicio, plan.vigencia_dias)
      const { data: np, error: cpE } = await supabase.from('clientes_planes').insert({ cliente_id: id, plan_id: plan.id, sesiones_totales: sesN, sesiones_usadas: 0, fecha_inicio: fechaInicio, fecha_fin: fechaFin, estado: 'activo' }).select('id').single()
      if (cpE) throw new Error(cpE.message)
      ...
      if (registrarPago && montoBase > 0) {
        const mF = esBs ? montoCalculadoBs : montoBase
        const { error: pE } = await supabase.from('pagos').insert({ fecha: fechaInicio, tipo_origen: 'plan', cliente_id: id, cliente_plan_id: np.id, concepto: `Renovación: ${plan.nombre} — ${cliente?.nombre}`, categoria: 'plan', monto: mF, metodo_pago_id: metodoPagoId, estado: 'pagado', notas: notasPago || null })
        if (pE) throw new Error(`Pago: ${pE.message}`)
      }
      if (baseC > 0) await registrarComision(np.id, empleadoId, id, baseC, fechaInicio, rpmV, entV)
      const porcRpm = baseC > 0 ? r2((rpmV / baseC) * 100) : 0
      await supabase.from('clientes_planes').update({ origen: 'manual', porcentaje_rpm: porcRpm, monto_base_comision: baseC }).eq('id', np.id)
      setSuccessMsg(`Plan renovado. ${sesN} sesiones totales.`)
      resetForm(); setModo(null); await fetchAll()
  ...
  async function handleCancelar(e: React.FormEvent) {
    ...
    try {
      await supabase.from('clientes_planes').update({ estado: 'cancelado' }).eq('id', planActivo.id)
      await supabase.from('entrenamientos').update({ estado: 'cancelado' }).eq('cliente_plan_id', planActivo.id).eq('estado', 'programado')
      await supabase.from('clientes_planes_eventos').insert({ cliente_plan_id: planActivo.id, cliente_id: id, tipo: 'cancelado', detalle: motivoCancelacion })
      if (pagoAlCancelar === 'no') {
        await supabase.from('pagos').update({ estado: 'anulado' }).eq('cliente_plan_id', planActivo.id).eq('estado', 'pagado')
        await supabase.from('comisiones_detalle').delete().eq('cliente_plan_id', planActivo.id).eq('estado', 'pendiente')
      }
      setSuccessMsg('Plan cancelado. Solo el plan más reciente fue cancelado.')
      resetForm(); setModo(null); await fetchAll()
      ...
      <form onSubmit={onSubmit} className="space-y-6">
          ...
          <Field label="Plan">
            <div className="flex gap-2">
              <select value={selectedPlanId} onChange={(e) => {
                ...
                {planes.map((p) => <option key={p.id} value={p.id} className="bg-[#11131a]">{p.nombre} · {p.sesiones_totales} ses. · {formatMoney(p.precio)}</option>)}
              </select>
            ...
            {mostrarCrearPlan && (
              <div className="mt-3 space-y-3 rounded-2xl border border-violet-400/20 bg-violet-400/5 p-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div><label className="mb-1 block text-xs text-white/55">Vigencia (días)</label><input type="number" min={1} value={nuevoPlanVigencia} onChange={(e) => setNuevoPlanVigencia(Number(e.target.value))} className={inputCls} /></div>
                </div>
                <div className="flex gap-2">
        ...
        {selectedPlan && (
          <Card className="border-white/10 bg-white/[0.02] p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
              <div><p className="text-xs text-white/45">Sesiones</p><p className="font-medium text-white">{selectedPlan.sesiones_totales}</p></div>
              {modo === 'renovar' && sesionesRestantes > 0 && <div><p className="text-xs text-white/45">+ Pendientes</p><p className="font-medium text-emerald-400">+{sesionesRestantes}</p></div>}
              <div><p className="text-xs text-white/45">Total</p><p className="font-medium text-white">{modo === 'renovar' ? selectedPlan.sesiones_totales + sesionesRestantes : selectedPlan.sesiones_totales}</p></div>
              <div><p className="text-xs text-white/45">Vence</p><p className="font-medium text-white">{formatDate(addDaysToDate(fechaInicio, selectedPlan.vigencia_dias))}</p></div>
            </div>
          </Card>
        )}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Entrenador">
            <select value={empleadoId} onChange={(e) => { setEmpleadoId(e.target.value); setFechaVistaCal(fechaInicio) }} className={inputCls}>
              ...
              {empleados.map((e) => <option key={e.id} value={e.id} className="bg-[#11131a]">{e.nombre}{e.especialidad ? ` · ${e.especialidad}` : ''}</option>)}
            </select>
          </Field>
        ...
        {empleadoId && (
          <div className="space-y-3">
            ...
            <CalendarioDisponibilidad empleadoId={empleadoId} fechaVista={fechaVistaCal} horaInicio={horaInicio} horaFin={horaFin} entrenamientosExistentes={entrenamientosEmpleado} onSelectHora={(hora) => setHoraInicio(hora)} />
          </div>
        )}
        ...
        {fechasPreview.length > 0 && (
          <Card className="border-violet-400/20 bg-violet-400/5 p-4">
            ...
            <div className="mt-2 flex max-h-24 flex-wrap gap-1.5 overflow-y-auto">
              {fechasPreview.slice(0, 30).map((f, i) => <span key={i} className="rounded-lg bg-violet-500/10 px-2 py-0.5 text-xs text-violet-400">{new Date(`${f}T00:00:00`).toLocaleDateString('es', { day: 'numeric', month: 'short' })}</span>)}
              {fechasPreview.length > 30 && <span className="text-xs text-violet-400/60">+{fechasPreview.length - 30} más</span>}
            </div>
          </Card>
        )}

        {/* ✅ Comisión: 3 campos reactivos */}
        <div className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Field label="Base comisión" helper={selectedPlan ? `Precio del plan: ${formatMoney(selectedPlan.precio)}` : 'Monto base a repartir'}>
              <input type="number" min={0} step="0.01" value={montoBaseComision || (selectedPlan?.precio ?? '')} onChange={(e) => handleBaseComisionChange(e.target.value)} className={inputCls} placeholder="0.00" />
            </Field>
            <Field label="RPM recibe" helper="Al editar, ajusta el del entrenador">
              <input type="number" min={0} step="0.01" value={montoRpm} onChange={(e) => handleMontoRpmChange(e.target.value)} className={inputCls} placeholder="0.00" />
            </Field>
            <Field label="Entrenador recibe" helper="Al editar, ajusta el de RPM">
              <input type="number" min={0} step="0.01" value={montoEntrenador} onChange={(e) => handleMontoEntrenadorChange(e.target.value)} className={inputCls} placeholder="0.00" />
            </Field>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <Card className="p-4"><p className="text-xs text-white/45">Base</p><p className="mt-1 text-lg font-semibold text-white">{formatMoney(baseV)}</p></Card>
            <Card className="border-violet-400/20 bg-violet-400/5 p-4">
          ...
          {desbal && (
            <Card className="border-amber-400/20 bg-amber-400/5 p-4">
              <p className="text-sm text-amber-300">⚠️ RPM ({formatMoney(rpmV)}) + Entrenador ({formatMoney(entV)}) = {formatMoney(sumaV)}, pero la base es {formatMoney(baseV)}. Ajusta los valores.</p>
            </Card>
          )}
        </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Método de pago">
                  <select value={metodoPagoId} onChange={(e) => setMetodoPagoId(e.target.value)} className={inputCls}>
                    ...
                    {metodosPago.map((m) => <option key={m.id} value={m.id} className="bg-[#11131a]">{m.nombre}{m.tipo ? ` · ${m.tipo}` : ''}</option>)}
                  </select>
                </Field>
                <Field label="Monto">
                  <div className="flex gap-2">
                    <input type="number" min={0} step="0.01" value={usarPrecioPlan ? (selectedPlan?.precio ?? '') : montoPersonalizado} readOnly={usarPrecioPlan} onChange={(e) => setMontoPersonalizado(e.target.value)} className={`${inputCls} ${usarPrecioPlan ? 'cursor-not-allowed opacity-60' : ''}`} placeholder="0.00" />
                    <button type="button" onClick={() => setUsarPrecioPlan((p) => !p)} className="shrink-0 rounded-2xl border border-white/10 bg-white/[0.03] px-3 text-xs text-white/60 hover:bg-white/[0.06]">{usarPrecioPlan ? 'Editar' : 'Plan'}</button>
                  </div>
                </Field>
              </div>
              {esBs && (
                <div className="space-y-4 rounded-2xl border border-amber-400/20 bg-amber-400/5 p-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <Field label={`Tasa BCV ${referenciaTasa}`}><input type="number" step="0.0001" min="0" value={tasaBCV} onChange={(e) => setTasaBCV(e.target.value)} placeholder="Ej: 36.52" className={inputCls} /></Field>
                    <Field label="Total en Bs"><input readOnly value={montoCalculadoBs > 0 ? formatBs(montoCalculadoBs) : '—'} className={`${inputCls} cursor-not-allowed opacity-60`} /></Field>
                  </div>
                </div>
              )}
              <Field label="Notas del pago"><textarea value={notasPago} onChange={(e) => setNotasPago(e.target.value)} rows={2} className={`${inputCls} resize-none`} placeholder="Notas opcionales..." /></Field>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button type="submit" disabled={saving} className="rounded-2xl border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/[0.12] disabled:opacity-60">{saving ? 'Guardando...' : submitLabel}</button>
          <button type="button" onClick={() => { setModo(null); resetForm() }} className="rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-3 text-sm font-semibold text-white/80 transition hover:bg-white/[0.06]">Cancelar</button>
        </div>
      </form>
    )
  }

  if (loading) return <div className="space-y-4"><p className="text-sm text-white/55">Clientes / Plan</p><h1 className="text-2xl font-semibold text-white">Plan del cliente</h1><Card className="p-6"><p className="text-sm text-white/55">Cargando...</p></Card></div>
  if (!cliente) return <div className="space-y-4"><p className="text-sm text-white/55">Clientes / Plan</p><h1 className="text-2xl font-semibold text-white">Plan del cliente</h1><Card className="p-6"><p className="text-sm text-rose-400">{errorMsg || 'No encontrado.'}</p></Card></div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className="space-y-6 xl:col-span-2">
          {!modo && (
            <Section title="Gestión del plan" description="Acciones disponibles.">
              <div className="grid gap-3 sm:grid-cols-3">
                <button type="button" onClick={() => { setModo('asignar'); resetForm() }} className="rounded-2xl border border-white/10 bg-white/[0.05] p-4 text-left transition hover:bg-white/[0.08]">
                ...
                <button type="button" disabled={!planActivo} onClick={() => { setModo('renovar'); resetForm() }} className="rounded-2xl border border-white/10 bg-white/[0.05] p-4 text-left transition hover:bg-white/[0.08] disabled:cursor-not-allowed disabled:opacity-40">
                  <p className="font-semibold text-white">Renovar plan</p>
                  <p className="mt-1 text-xs text-white/45">{planActivo ? `${sesionesRestantes} ses. pendientes se suman` : 'Sin plan activo'}</p>
                </button>
                <button type="button" disabled={!planActivo} onClick={() => { setModo('cancelar'); resetForm() }} className="rounded-2xl border border-rose-400/20 bg-rose-400/5 p-4 text-left transition hover:bg-rose-400/10 disabled:cursor-not-allowed disabled:opacity-40">
          ...
          {modo === 'renovar' && (
            <Section title="Renovar plan" description="Renueva sumando sesiones pendientes.">
              {sesionesRestantes > 0 && <Card className="mb-4 border-emerald-400/20 bg-emerald-400/5 p-4"><p className="text-sm text-emerald-300">✓ Se sumarán {sesionesRestantes} sesiones pendientes.</p></Card>}
                ...
                <Field label="¿El cliente realizó el pago?">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <button type="button" onClick={() => setPagoAlCancelar('si')} className={`rounded-2xl border p-3 text-left transition ${pagoAlCancelar === 'si' ? 'border-emerald-400/30 bg-emerald-400/10' : 'border-white/10 bg-white/[0.02] hover:bg-white/[0.04]'}`}>
                      ...
                      <p className="mt-1 text-xs text-white/45">La comisión se mantiene pendiente de pagar al entrenador</p>
                    </button>
                    <button type="button" onClick={() => setPagoAlCancelar('no')} className={`rounded-2xl border p-3 text-left transition ${pagoAlCancelar === 'no' ? 'border-rose-400/30 bg-rose-400/10' : 'border-white/10 bg-white/[0.02] hover:bg-white/[0.04]'}`}>
                      ...
                      <p className="mt-1 text-xs text-white/45">Se anula el pago y la comisión no se suma al entrenador</p>
                    </button>
                  </div>
                </Field>
                {errorMsg && <Card className="p-4"><p className="text-sm text-rose-400">{errorMsg}</p></Card>}
                <div className="flex gap-3">
                  <button type="submit" disabled={saving} className="rounded-2xl border border-rose-400/20 bg-rose-400/10 px-5 py-3 text-sm font-semibold text-rose-300 transition hover:bg-rose-400/15 disabled:opacity-60">{saving ? 'Cancelando...' : 'Confirmar cancelación'}</button>
        ...
        <div className="space-y-4">
          <Section title="Plan activo" description="Estado actual.">
            {!planActivo ? <p className="text-sm text-white/55">Sin plan activo.</p> : (
              <div className="space-y-4">
                <div>
                ...
                <div>
                  <div className="mb-1 flex justify-between text-xs text-white/45"><span>Progreso</span><span>{progresoUso}%</span></div>
